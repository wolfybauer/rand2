...
# added sprite offset + position tracks
...
func _create_meta_tracks(sprite: Node, player: AnimationPlayer, animation: Animation):
	...
	var offset_track = _get_property_track_path(player, sprite, "offset")
	var offset_track_index = _create_track(sprite, animation, offset_track)
	animation.track_insert_key(offset_track_index, 0, Vector2.ZERO)
	
	var position_track = _get_property_track_path(player, sprite, "position")
	var position_track_index = _create_track(sprite, animation, position_track)
	animation.track_insert_key(position_track_index, 0, Vector2.ZERO)
